...
await agent.register({
  deposit: "6000000000000000000000", // optional yoctoNEAR; the contract refunds any excess
...
|-----------|-------------|
| `deposit` | Attached deposit in **yoctoNEAR** when the call must include a storage stake: first-time registration in auto mode, or whenever `forceDeposit` is `true`. If omitted in those cases, the client uses **`4860000000000000000000` yoctoNEAR** (0.00486 NEAR — the contract's storage cost). The contract requires *at least* this and **refunds any excess**, so attaching more is safe; raise this if your contract stores more per agent. |
...
|----------------|----------|
| *Omitted* or `undefined` | The client calls **`get_agent`** for the agent’s account ID. If the agent **is not** registered (`null`), it attaches **`deposit` or the default** (0.00486 NEAR in yocto). If the agent **is** already registered (re-registration / refresh), it attaches **no** deposit (`0`), because the contract does not charge extra storage for an existing key. |
| `true` | **Skips** `get_agent`. Always attaches **`deposit` or the default** (0.00486 NEAR yocto). Use when you know the contract expects a storage stake regardless of prior state — safe on a re-registration too, since the contract refunds the deposit when no storage is needed. |
