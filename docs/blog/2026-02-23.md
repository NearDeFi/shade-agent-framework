...
> [!CAUTION]
> **Upgrade immediately** — The previous version of the Shade Agent Framework has **known critical vulnerabilities**. You should upgrade your agent to 2.0 as soon as possible.
...
To learn how to install, configure, and use the API, see the **[Shade Agent API reference](../reference/api.md)**.
...
To learn how to install the CLI, use each command, and configure `deployment.yaml`, see the **[Shade Agent CLI reference](../reference/cli.md)**.
...
The reference agent contract also now uses a more robust external library for attestation verification (the [shade-attestation crate](../../shade-attestation/README.md)). Instead of approving the codehash of a single Docker image, the contract now requires you to approve a set of measurements for more in-depth verification and a list of PPIDs that set the physical hardware the agent can run on. Local mode now requires you to whitelist the account ID of the agent you want to run locally, blocking any other account ID from controlling the contract for more consistent behavior when testing.

To walk through the contract flow, initialization, attestation verification, and how to add your own agent-gated functions, see the **[Agent Contract reference](../reference/agent-contract.md)**.

---

## Next Steps

Start your migration by **cloning the template** in the [quickstart](../getting-started/quickstart-deploying.md) to explore the new setup.
