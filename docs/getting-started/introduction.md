...
![Shade Agent Architecture](../assets/shade-agent-stack-diagram.png)
...
Once registered, the **agent** can call the agent-gated functions on the agent contract. An example of this is the `request_signature` function, enabling it to sign transactions on behalf of the Shade Agent. `request_signature` leverages [chain signatures](https://docs.near.org/chain-abstraction/chain-signatures) for decentralized key management, allowing the Shade Agent to hold assets and sign transactions on nearly any chain.
