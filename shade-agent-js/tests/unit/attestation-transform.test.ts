...
import { createMockDstackTcbInfo, createMockQuoteCollateral } from '../mocks/tee-mocks';
...
describe('attestation-transform', () => {
  ...
  describe('transformCollateral', () => {
    it('should transform complete raw collateral to Collateral structure', () => {
      ...
      expect(result.tcb_info_signature).toEqual([115, 105, 103, 110, 97, 116, 117, 114, 101, 49]);
      expect(result.qe_identity_issuer_chain).toBe('chain3');
      expect(result.qe_identity).toBe('qe_identity_json');
      expect(result.qe_identity_signature).toEqual([115, 105, 103, 110, 97, 116, 117, 114, 101, 50]);
    });
