...
| `test_register_agent_reregister_without_storage_deposit_integration` | After a successful first registration, `register_agent` again with no deposit succeeds. **Ignored by default** — run with `-- --ignored` after rebuilding `target/near/shade_contract_template.wasm` from current sources so the deployed WASM matches this tree. |
