...
4. **Tests to add** - every code path introduced or changed needs a test:
   ...
   - Hostile inputs on every untrusted, caller-controlled boundary the change touches — planned per the trust-boundary checklist in `.claude/project-specifics/project-specific-concerns.md`; a break on an untrusted boundary is a real bug, so its test must fail on the unfixed code
...
**Falsify the plan before showing it** (default; skip only for a trivial or docs-only change with no real interaction surface). Once the plan is drafted, dispatch one independent subagent (Agent tool) to try to prove it *wrong* — give it the full inlined plan, the change's likely blast radius, and `.claude/project-specifics/project-specific-concerns.md`, and ask it: "Find the highest-impact way this plan is wrong or incomplete — a violated project-specific concern or universal rule, an untrusted input left untested, a required consumer / doc / test update the change misses, or a bad interaction with code the plan doesn't touch. Say what would confirm vs refute each concern, then report what you found." **Then amend the plan to address every real finding** — the point of this pass is to harden the plan before it's shown, not just to surface problems. When you present the revised plan, note in one line what you changed as a result (or that the review found nothing to change).
