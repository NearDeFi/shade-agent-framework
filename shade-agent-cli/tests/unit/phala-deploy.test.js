 ...
 *    the helper renders a red error and exits 1 (regression for SAF-002).
 *
 * Notes:
...
describe("deployToPhala", () => {
  ...
  // dstack_version is now required (no silent default to "0.5.8").
  it("throws when dstackVersion is missing", async () => {
    await expect(
      deployToPhala({
        appName: "my-app",
        apiKey: "key",
    ...
    ).rejects.toThrow(/dstack_version/);
  });

  // instance_type is now required (no silent default to "tdx.small").
  it("throws when instanceType is missing", async () => {
    await expect(
      deployToPhala({
        appName: "my-app",
        apiKey: "key",
    ...
    ).rejects.toThrow(/instance_type/);
  });

  // appName must be longer than 3 chars (Docker tag rule + Phala constraint).
  it("throws on appName.length <= 3", async () => {
    await expect(
      deployToPhala({
        appName: "abc",
        apiKey: "key",
    ...
    ).rejects.toThrow(/longer than 3 characters/);
  });

  // SAF-002 regression: when provision.compose_hash differs from the locally
