...
| [example-10.yaml](./example-10.yaml) | **Locked-down telemetry.** First-time TEE deployment with `public_logs` and `public_sysinfo` both disabled, so `/logs/<container>` and `/metrics` are not publicly reachable. Produces a different compose hash from the default (telemetry-on) examples. |
