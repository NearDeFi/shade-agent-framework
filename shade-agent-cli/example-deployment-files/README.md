...
| [example-10.yaml](./example-10.yaml) | **Locked-down telemetry.** First-time TEE deployment with `public_logs` and `public_sysinfo` both disabled. |
| [example-11.yaml](./example-11.yaml) | **Self-hosted dstack server.** First-time TEE deployment to your own TDX box over SSH instead of Phala Cloud, using `tee_config.server`. |
