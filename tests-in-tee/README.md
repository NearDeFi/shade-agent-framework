...
| `registration-intel-pcs`      | Same as `successful-registration`, but the agent is created with `pccsEndpoints: ["https://api.trustedservices.intel.com"]`, so the quote collateral comes from Intel's PCS alone rather than the default Phala-PCCS-then-Intel ladder (Intel serves no `rootcacrl`, so the root CRL is fetched from the certificate's distribution point). The script checks there are no registration or call errors and that the agent appears as registered on the contract. |
| `registration-phala-pccs`     | Same as `successful-registration`, but the agent is created with `pccsEndpoints: ["https://pccs.phala.network"]`, so the quote collateral comes from Phala's PCCS alone. Together with `registration-intel-pcs` this proves each default endpoint on its own, rather than behind the ladder's fall-through. The script checks there are no registration or call errors and that the agent appears as registered on the contract. |
